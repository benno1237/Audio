...
class EmptyPauseTimerManager(CacheBase):
    __slots__ = (
        "_config",
        "bot",
        "enable_cache",
    ...
    async def get_global(self) -> int:
        ...
        if self.enable_cache and None in self._cached_global:
            ...
            ret = await self._config.emptypause_timer()
            self._cached_global[None] = ret
        return ret

    async def set_global(self, set_to: Optional[int]) -> None:
        if set_to is not None:
            await self._config.emptypause_timer.set(set_to)
            self._cached_global[None] = set_to
        else:
            await self._config.emptypause_timer.clear()

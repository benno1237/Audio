...
class PreferLyricsManager(CacheBase):
    __slots__ = (
        "_config",
        "bot",
        "enable_cache",
    ...
    async def get_global(self) -> bool:
        ...
        if self.enable_cache and None in self._cached_global:
            ...
            ret = await self._config.prefer_lyrics()
            self._cached_global[None] = ret
        return ret

    async def set_global(self, set_to: Optional[bool]) -> None:
        if set_to is not None:
            await self._config.prefer_lyrics.set(set_to)
            self._cached_global[None] = set_to
        else:
            await self._config.prefer_lyrics.clear()
            self._cached_global[None] = self._config.defaults["GLOBAL"]["prefer_lyrics"]

...
class AutoLyricsManager(CacheBase):
    __slots__ = (
        "_config",
        "bot",
        "enable_cache",
    ...
    async def set_guild(self, guild: discord.Guild, set_to: Optional[bool]) -> None:
        ...
        if set_to is not None:
            ...
            self._cached_guild[gid] = self._config.defaults["GUILD"]["auto_lyrics"]
    ...
    async def set_global(self, set_to: Optional[bool]) -> None:
        if set_to is not None:
            ...
            self._cached_global[None] = self._config.defaults["GLOBAL"]["auto_lyrics"]
